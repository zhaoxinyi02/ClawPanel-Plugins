...
export const QQConfigSchema = z.object({
  ...
  // 唤醒与限流配置
  rateLimit: z.object({
    // 唤醒概率 (0-100)，仅对群消息生效，私聊始终100%
    wakeProbability: z.number().min(0).max(100).default(10).describe("群消息唤醒概率(0-100%)，私聊始终100%"),
    // 消息发送最小间隔（秒）
    minIntervalSec: z.number().min(0).default(0).describe("消息发送最小间隔(秒)，0=不限制"),
    // 唤醒触发方式
    wakeTrigger: z.object({
      mode: z.enum(["any", "all"]).default("any").describe("多条件生效逻辑: any=满足任一, all=全部满足"),
      mentionName: z.boolean().default(true).describe("提及Bot名字时唤醒"),
      atBot: z.boolean().default(true).describe("@Bot时唤醒"),
      keywords: z.array(z.string()).default([]).describe("关键词列表，消息包含任一关键词时唤醒"),
      directMessage: z.boolean().default(true).describe("私聊消息始终唤醒"),
    }).optional(),
    // API报错去重
    errorDedup: z.object({
      enabled: z.boolean().default(true).describe("是否启用API报错去重"),
      thresholdSec: z.number().min(0).default(300).describe("同一报错的去重时间阈值(秒)"),
    }).optional(),
  }).optional(),
});
