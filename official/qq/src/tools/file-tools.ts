...
// NapCat 运行在 Docker 容器中，无法访问宿主机本地路径
// 需要将宿主机路径转换为文件服务器的 HTTP URL（通过 Docker 网桥访问）
const FILE_SERVER_BASE_URL = "http://172.17.0.1:18790";
const HOST_DIRS = ["/home/zhaoxinyi/openclaw/work", "/home/zhaoxinyi/.openclaw", "/tmp"];

function toFileServerUrl(filePath: string): string {
  if (filePath.startsWith("http://") || filePath.startsWith("https://") || filePath.startsWith("base64://")) {
    return filePath;
  }
  if (HOST_DIRS.some(d => filePath.startsWith(d + "/") || filePath === d)) {
    const url = FILE_SERVER_BASE_URL + "/file?path=" + encodeURIComponent(filePath);
    console.log("[QQ] file-tools: converted path -> URL: " + filePath + " -> " + url);
    return url;
  }
  return filePath;
}
...
export const fileTools: ToolDefinition[] = [
  {
    ...
    execute: async (client, params) => {
      await client.uploadGroupFile(params.group_id, toFileServerUrl(params.file), params.name, params.folder || "");
  ...
  },
  {
    ...
    execute: async (client, params) => {
      await client.uploadPrivateFile(params.user_id, toFileServerUrl(params.file), params.name);
