...
// File server config for converting local paths to URLs accessible by NapCat (Docker)
const FILE_SERVER_BASE = "http://172.17.0.1:18790";
const SERVED_DIRS = ["/home/zhaoxinyi/openclaw/work", "/home/zhaoxinyi/.openclaw", "/tmp"];
function localPathToUrl(filePath: string): string {
  if (filePath.startsWith("http://") || filePath.startsWith("https://")) return filePath;
  if (SERVED_DIRS.some(d => filePath.startsWith(d + "/") || filePath === d)) {
    return FILE_SERVER_BASE + "/file?path=" + encodeURIComponent(filePath);
  }
...
export const messageTools: ToolDefinition[] = [
  ...
  },
  {
    ...
    description: "以动画表情格式发送图片/贴纸（而非普通图片）。支持传入：1) HTTP URL 2) 本地文件路径（如 /home/zhaoxinyi/openclaw/work/emojis/xxx.png，会自动转为HTTP URL）。适合发送保存在emojis文件夹中的表情包。",
    parameters: {
      type: "object",
      properties: {
        target_type: { type: "string", enum: ["private", "group"], description: "目标类型" },
        target_id: { type: "number", description: "目标QQ号或群号" },
        file: { type: "string", description: "表情图片URL或本地文件路径（如 /home/zhaoxinyi/openclaw/work/emojis/xxx.png）" },
      },
      required: ["target_type", "target_id", "file"],
    },
    execute: async (client, params) => {
      const fileUrl = localPathToUrl(params.file);
      const message = [
        { type: "image", data: { file: fileUrl, sub_type: 1, summary: "[动画表情]" } }
      ];
      if (params.target_type === "group") {
        const result = await client.sendGroupMsg(params.target_id, message as any);
        return { success: true, message_id: result.data?.message_id };
      } else {
        const result = await client.sendPrivateMsg(params.target_id, message as any);
        return { success: true, message_id: result.data?.message_id };
      }
    },
  },
  {
    ...
    description: "获取QQ收藏的自定义表情列表（收藏表情）。返回表情URL列表。",
    parameters: {
      type: "object",
      properties: {
    ...
    execute: async (client, params) => {
      ...
      return { faces: result.data || [], count: (result.data || []).length };
    },
  },
];
